export const cashfreePaymentsConfig = (env: any) => {
	if (!env) {
...
export const cashfreePayoutsConfig = (env: any) => {
	if (!env) {
		throw new Error('Environment variables not accessible');
	}

	if (
		!env.CASHFREE_PAYOUTS_CLIENT_ID ||
		!env.CASHFREE_PAYOUTS_CLIENT_SECRET
	) {
		throw new Error('Required Cashfree environment variables are missing');
	}

	return {
		clientId: env.CASHFREE_PAYOUTS_CLIENT_ID,
		clientSecret: env.CASHFREE_PAYOUTS_CLIENT_SECRET,
		environment: env.CASHFREE_ENVIRONMENT || 'sandbox',
		baseUrl:
			env.CASHFREE_ENVIRONMENT === 'production'
				? 'https://api.cashfree.com'
